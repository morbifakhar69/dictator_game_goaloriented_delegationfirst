...
class Decision(Page):
    ...
    def vars_for_template(self):
        ...
        elif current_part==2 or (current_part ==3 and not self.player.delegate_decision_optional ):
            current_player = self.player.in_round(display_round)
            allocation=current_player.allocation
        return {
    ...
    def before_next_page(self):
        ...
        if current_part == 2  :  # Part 1 logic or Part 3 with manual with manual decisions and timer
    ...
    def save_player_data(self):
            ...
            for pl in self.player.in_all_rounds():          # built-in oTree helper
                rows.append({
                    ...
                    "orientation_history": pl.field_maybe_none('orientation_history'),
            ...
            static_columns = [ "prolific_id","gender", "age", "occupation", "ai_use", "task_difficulty", "feedback",'random_payoff_part','experiment','orientation_history','comprehension_attempts','incorrect_answers','is_excluded']
